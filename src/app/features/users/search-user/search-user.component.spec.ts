...
describe('SearchUserComponent', () => {
  ...
  it('should be no result with empty value', fakeAsync(() => {
    component.f.keyword.setValue('');
    component.onSubmit();
    tick();
    expect(component.users.length).toEqual(0, 'users result should be empty');
  }));

  it('should display users with name infor contains Name search value', fakeAsync(() => {
    component.f.searchMethod.setValue(SearchMethod.Name);
    const keyword = getTestUsers()[0].name;
    component.f.keyword.setValue(keyword);
    component.onSubmit();
    tick();
    expect(component.users.length).toBeGreaterThan(0, 'should find matched users');

    component.users.forEach(user => {
      expect(user.name.toLowerCase()).toContain(keyword.toLowerCase(), `should find users with name contains '` + keyword + `'` );
    });
  }));

  it('should display users with username infor contains Username search value', fakeAsync(() => {
    component.f.searchMethod.setValue(SearchMethod.Username);
    ...
    component.users.forEach(user => {
      expect(user.username.toLowerCase()).toContain(keyword.toLowerCase(), `should find users with username contains '` + keyword + `'` );
    });
  }));

  it('should display users with email infor match exactly Email search value', fakeAsync(() => {
    component.f.searchMethod.setValue(SearchMethod.Email);
    ...
    component.users.forEach(user => {
      expect(user.email.toLowerCase()).toEqual(keyword.toLowerCase(), `should find users with email is '` + keyword + `'` );
    });
  }));

  it('should display Not Matching message with no result for search value', fakeAsync(() => {
    component.f.searchMethod.setValue(SearchMethod.Email);
  ...
  it('should display users with Name, Username, Email information', fakeAsync(() => {
    component.f.searchMethod.setValue(SearchMethod.Email);
  ...
  it('should display error message when UserService.search fails', fakeAsync(() => {
    ...
    const searchSpy = spyOn(userServiceSpy, 'search').and.returnValue(throwError('UserService test failure'));
  ...
  it('should search normal with specical character in search value', fakeAsync(() => {
    component.f.searchMethod.setValue(SearchMethod.Name);
    const keyword = '{*%!';
    component.f.keyword.setValue(keyword);
